...
public class ImageLoader {
    ...
    public void setBlurImage(final Activity activity, final String url, final View targetView, ImageView goneView) {
     ...
     */
    private void applyBlur(final Activity activity, final int radius, final Bitmap bitmap, final String url, final View targetView) {
            ...
            public void run() {
                ...
                try {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            targetView.setBackground(new BitmapDrawable(mBitmap));
    ...
    public static void displayResImg(Context act, int resId, ImageView imageView) {
        //DiskCacheStrategy.ALL 硬盘缓存策略为缓存原图，用于显示不同的大小的ImageView上会更快
        Glide.with(act).load(resId).diskCacheStrategy(DiskCacheStrategy.ALL).error(R.mipmap.recyclerview_item).into(imageView);
    }
    /**
     * 加载头像图片
     *
    ...
    public static void displayImg(Context context, String url, ImageView imageView) {
        //DiskCacheStrategy.ALL 硬盘缓存策略为缓存原图，用于显示不同的大小的ImageView上会更快
        Glide.with(context).load(url).diskCacheStrategy(DiskCacheStrategy.ALL).error(R.mipmap.recyclerview_item).into(imageView);
    }
